$path = "backend\Controllers\IAController.cs"
$content = Get-Content -Path $path -Raw

# Replace using
$content = $content -replace "using System.Linq;", "using System.Linq;`r`nusing Google.Apis.Auth.OAuth2;"

# Replace AnalizarFactura URL
$target1 = [regex]::Escape('                var apiKey = "AIzaSyC6M26s6rDMecZpX-GDKwzzvPQSRM18OxM";
                var url = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={apiKey}";

                var content = new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json");
                var response = await _httpClient.PostAsync(url, content);')
$rep1 = '                var auth = await GetVertexAuthAsync();
...
# Replace Chatbot URL
$target2 = [regex]::Escape('            var apiKey = "AIzaSyC6M26s6rDMecZpX-GDKwzzvPQSRM18OxM";
            var geminiUrl = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={apiKey}";')
$rep2 = '            var auth = await GetVertexAuthAsync();'
$content = $content -replace $target2, $rep2

# Replace Chatbot SQL post
$target3 = [regex]::Escape('                var sqlPayload = new { contents = new[] { new { parts = new[] { new { text = promptSQL } } } } };
...
$rep3 = '                var sqlPayload = new { contents = new[] { new { parts = new[] { new { text = promptSQL } } } } };
...
$target4 = [regex]::Escape('                var finalPayload = new { contents = new[] { new { parts = new[] { new { text = promptFinal } } } } };
...
$rep4 = '                var finalPayload = new { contents = new[] { new { parts = new[] { new { text = promptFinal } } } } };
